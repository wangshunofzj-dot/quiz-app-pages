...
function renderOptions(q, savedAnswer) {
  ...
  const submitted = state.checked.get(q.question_id)?.isDone;

  dom.optionsWrap.innerHTML = options
    .map((opt) => {
      const checked =
        inputType === "checkbox"
          ? selectedSet.has(opt.letter)
          : normalizedSaved === normalizeAnswer(opt.letter, q.question_type);
      return `<label class="option-item">
        <input type="${inputType}" name="answer" value="${opt.letter}" ${checked ? "checked" : ""} ${submitted ? "disabled" : ""} />
...
function renderQuestion() {
  ...
  dom.checkBtn.disabled = Boolean(checked?.isObjective && checked?.isDone);
...
function checkAnswer() {
  const q = getCurrentQuestion();
  if (!q) return;
  if (OBJECTIVE_TYPES.has(q.question_type) && state.checked.get(q.question_id)?.isDone) return;
